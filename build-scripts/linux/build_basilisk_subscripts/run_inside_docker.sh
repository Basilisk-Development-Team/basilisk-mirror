...
cmake -S runtimes -B build \
  ...
  -DCMAKE_C_FLAGS="-fPIC -fvisibility=hidden -nostdinc++ -nostdlib++" \
  -DCMAKE_CXX_FLAGS="-fPIC -fvisibility=hidden -fvisibility-inlines-hidden -nostdinc++ -nostdlib++" \
...
if [ "$(uname -m)" = "x86_64" ]; then
  ...
  cmake -S runtimes -B build-i686 \
    ...
    -DCMAKE_C_FLAGS="-m32 -fPIC -fvisibility=hidden -nostdinc++ -nostdlib++" \
    -DCMAKE_CXX_FLAGS="-m32 -fPIC -fvisibility=hidden -fvisibility-inlines-hidden -nostdinc++ -nostdlib++" \
    -DCMAKE_ASM_FLAGS="-m32 -fPIC" \
    -DCMAKE_EXE_LINKER_FLAGS="-fuse-ld=lld -m32 -nostdlib++"
